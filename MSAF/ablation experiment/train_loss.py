...
"""
Run:
python train_loss.py --dataset ./data/HFBTP.csv --stop_metric both --use_gpu
"""
...
class LitHybrid(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        tgt_t = y[:, 0]
        tgt_l = y[:, 1]
        loss = F.mse_loss(pred_t, tgt_t) + F.mse_loss(pred_l, tgt_l)
...
def main():
    ...
    print("="*60)
    print("Hybrid FTT + RaftGAT - Dual Best Model Tracking (Equal-Weight Loss)")
    ...
    df_res.to_csv("results/dual_best_5fold_results_equalweight.csv", index=False)
    ...
    pd.DataFrame(summary).to_csv("results/dual_best_summary_equalweight.csv", index=False)
